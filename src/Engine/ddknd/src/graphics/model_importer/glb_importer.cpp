...
// ==================================== alias ====================================
namespace glType = ::ddknd::graphics::internal::type;
...
namespace
{
    ...
    Mat4f ReadNodeLocalMatrix(const cgltf_node* n)
    {
        ...
        Vec3f T = n->has_translation 
            ? Vec3f{n->translation[0], n->translation[1], n->translation[2]} 
            : Vec3f{0, 0, 0};

        Quatf R = n->has_rotation 
            ? Quatf{n->rotation[3], n->rotation[0], n->rotation[1], n->rotation[2]} 
            : Quatf::Identity();

        Vec3f S = n->has_scale 
            ? Vec3f{n->scale[0], n->scale[1], n->scale[2]} 
            : Vec3f{1, 1, 1};
...
namespace
{
    ...
    int ReadPrimitives(const cgltf_data* g, const cgltf_primitive& srcPrim, ModelImportData& out)
    {
        ...
        for(std::size_t i = 0; i < vcount; i++)
        {
            ...
            if(!normals.empty()) v.normal = normals[i];
            if(!uvs.empty()) v.texCoords = uvs[i];
            if(!tangents.empty()) v.tangent = tangents[i];
            if(!joints.empty()) v.joints = joints[i];
            if(!weights.empty()) v.weights = weights[i];
    ...
    void ReadSkins(const cgltf_data* g, ModelImportData& out)
    {
        ...
        for(cgltf_size si = 0; si < g->skins_count; si++)
        {
            ...
            dst.skeletonRootNode = src->skeleton 
                ? IndexOf(g->nodes, g->nodes_count, src->skeleton) 
                : -1;
    ...
    void ReadAnimations(const cgltf_data* g, ModelImportData& out)
    {
        ...
        for(cgltf_size ai = 0; ai < g->animations_count; ai++)
        {
            ...
            for(cgltf_size ci = 0; ci < srcAnim->channels_count; ci++)
            {
                ...
                ImportChannel ch{};
                ch.targetNode = IndexOf(g->nodes, g->nodes_count, srcCh.target_node);
