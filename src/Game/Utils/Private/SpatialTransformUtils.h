...
namespace Game::Utils
{
	...
	glm::vec2 GetLogicDirectionFromCursorToTarget(const glm::vec2& screenMousePos, const Engine::Graphics::RenderContext& renderContext,
		const glm::vec2& playerLogicPosXZ // Character position in logical coordinates
		);

	// スクリーン座標(マウスポインタなど)から、論理XZ平面での座標を得る
	// glm::vec2 ProjectScreenToLogicXZPlane(const glm::vec2& screenMousePos, const Engine::Graphics::RenderContext& renderContext);
}