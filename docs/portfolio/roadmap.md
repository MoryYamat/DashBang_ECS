...
- 入力源に固有の特性とその違いを State / Resolver やその前段階で吸収できるのか
- 各 Stage 間のフィードバックループの扱い

### State / Resolver の構造的ボトルネック

現行実装は、Requested Intent を State / Resolver を通して、Gameplay Logic へ渡す構造にしている．

この構造によって、入力要求と状態制約、動作を分離しやすくなった一方で、規模が大きくなると、  
State-Machine の状態数や Resolver 責務が増大・複雑化する可能性がある．
...
Asset 定義、Animation Bind、Hitbox などの Actor 定義を代表として、ほかにも、  
Terrain や Scene、などの大きな時空間におけるデータの定義や編集についても考えていく必要がある．


## 今後の改善方針

### AI / Network Input に関する Intent Pipeline の検証

まずは、AI Input の最小実装によってPlayer Input と AI が同じ Intent Pipeline によって動作するか検証する．  
また、個別事例に特有の制約に対して対応できるかについての検証も必要である．

これによって、現在の Input Process Pipeline が Local Player 専用の構造になっていないかを確認する．

### State / Resolver の分割とデータ駆動化
...
そのため、まず基本部分に小さいテストを追加しつつ、内部実装の変更時、破壊的変更を検出できるような構成にすることを目指したい．
...
- State / Resolver の優先順位制御の
- 外部 Animation Asset を既存 Skeleton に適応する
- Animation Blend

これにより、設計の失敗を早期に発見し、手戻りや不確実性に関するコストを減らすことを身につけなければいけない．# Roadmap
...
Asset定義、Animation Bind、Hitbox などの Actor 定義を代表として、ほかにも、  
Terrain や Scene などの大きな時空間におけるデータの定義や編集についても考えていく必要がある．
