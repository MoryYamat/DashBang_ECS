...
ECS を基盤 Architecture として考えた理由の一つは、学習と拡張のしやすさにあると言える．  
...
このプロジェクトにおける、ECS のコアの実装は初期実装と現行実装の2種類が存在する．  
現行実装では、初期実装の課題を受けて改善を目指した．

具体的には、  
機能追加や内部実装の変更が他の機能へ波及しにくい構造にすることを重視した．  
...
これによって、Component の配置が変わる可能性 があり、  
長期間 Component へのポインタや参照を保持する設計とは相性が悪くなっている．  
この実装では、Component 参照の寿命管理や、将来的な並列実行時の安全性に影響をもたらすことが考えられる．
